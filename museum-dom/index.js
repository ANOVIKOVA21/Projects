...
const container = document.querySelector('.welcome__img');
...
const arrows = document.querySelectorAll('.welcome__arrow');
let indexCurImg = 0;
let currentImg = images[indexCurImg];

slider.addEventListener('click', (event) => {
  const target = event.target;
  if (!target.closest('.welcome__arrow') && !target.closest('.welcome__square'))
    return;
...
inputDate.addEventListener('click', () => {
  inputDate.style.background =
    'url(assets/svg/arrow_top.svg) no-repeat center right';
});
inputDate.addEventListener('blur', () => {
  inputDate.style.background =
    'url(assets/svg/arrow_down.svg) no-repeat center right';
});
...
menuButton.addEventListener('click', () => {
  if (menuButton.dataset.isOpen === 'true') {
    ...
    menuButton.style.background =
      'url(assets/svg/burger.svg) no-repeat right transparent';
    ...
    menu.hidden = false;
    menuButton.style.background =
      'url(assets/svg/close-white.svg) no-repeat right transparent';
...
function initComparisons() {
  ...
  function compareImages(img) {
    ...
    img.style.width = w / 2 + exploreSlider.style.width / 2 + 'px';
    /*create slider:*/
    ...
    function slide(x) {
      ...
      exploreSlider.style.left =
        img.offsetWidth - exploreSlider.offsetWidth / 2 + 'px';
    }
  }
}
...
const btnPlayLarge = document.querySelector('.video__play__large');
const volumeProgress = document.querySelector('.video__volume__progress');
...
controls.addEventListener('click', (event) => {
  if (
    event.target.closest('.video__play') ||
    event.target.closest('.video__play__large')
  )
    videoPlayPause();
...
function countSumOfTicket() {
  let sum =
    getValue(ticketBasic) * prices[typeOfTicket] +
    (getValue(ticketSenior) * prices[typeOfTicket]) / 2;
